...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Bad Request,so sorry")
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 Temporarily Unavailable (sad)")
        else:
            self.sendResponse("500 Server Internal Error hups")
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 Strictly Not Acceptable")
        else:
            self.sendResponse("500 Server Internal Error hups")
    ...
    def handle(self):
        ...
        data = self.request[0].decode('cp1252')
